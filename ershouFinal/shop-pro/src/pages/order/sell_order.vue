  ...
  <view class="page-box">
    ...
    <view class="order" v-for="(item, index) in list" :key="item.orderId" style="margin-left: 16px">
      ...
      <view class="bottom" style="margin-top: 10px">
        <u-button v-if="item.orderStatus === '1'" @click="post(item)" class="logistics btn" type="info">发货</u-button>
        <u-button @click="toLook(item)" class="logistics btn" type="info">查看</u-button>
        <u-button @click="cancelOrder(item)" class="evaluate btn" type="error" v-if="item.orderStatus !== '2' && item.orderStatus !== '3' && item.orderStatus !== '4'">取消</u-button>
      </view>
      <u-divider text="分割线" style="margin-left: 16px; margin-right: 16px"></u-divider>
    </view>

    <!-- 发货提示框 -->
    <u-popup :mask-close-able="false" border-radius="15" width="70%" height="120px" v-model="post_dialog_show" mode="center">
      <view style="padding: 10px 10px 10px 10px;">
        <view>请输入发货单号</view>
        <u-input v-model="pageData.sendNo" placeholder="快递单号"/>
      </view>
      <view class="conBtn">
        <u-button @click="post_dialog_show = false" style="margin-right: 15px;" type="info">取消</u-button>
        <u-button @click="doPostOrder" style="margin-left: 15px;" type="error">确定</u-button>
      </view>
    </u-popup>
...
import {getMyOrderApi, deleteOrderApi, getSellOrderApi, postOrder} from '../../api/order.js'
import {ref} from "vue";
import {listCart} from "../../api/cart";

const list = ref([])

const post_dialog_show = ref(false)
...
const cancelOrder = (item) => {
  ...
  deleteOrderApi(item.orderId).then(res => {
    if (res && res.code == 200) {
      getPageList()
...
const doPostOrder = () => {
  if (!pageData.sendNo){
    uni.showToast({title: '请输入发货单号', icon: 'error', duration: 1000})
    return
  }
  uni.showLoading({title: '发货中'});
  postOrder(pageData.selectOrderId, pageData.sendNo).then(res => {
    if (res && res.code == 200) {
